...
public class UserAccountService(IdentityDbContext context, UserManager<ApplicationUser> userManager)
    : IUserAccountService
{
    ...
    /// <summary>
    /// Runs the deletion pre-checks owned by this bounded context. Data owned by other
    /// services (such as the orders a cashier is referenced on) is not consulted.
    /// </summary>
    public Task<(bool Success, string[] Errors)> TestCanDeleteUserAsync(string userId)
    {
